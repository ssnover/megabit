...
SECTIONS {
    ...
    * output region or load region in those user sections! */
    ...
    /* Note that this pads the FLASH_BOOT usage to 32 byte alignment. */
    ...
    * output region or load region in those user sections! */
    ...
    /* NVS region to allow the bootloader to write to the tables */
    .nvs (NOLOAD) : {
        KEEP(*(.nvs))
    } > NVS
}
...
/* # Alignment checks */
ASSERT(ORIGIN(FLASH_BOOT) % 4 == 0, "
ERROR: the start of the FLASH_BOOT region must be 4-byte aligned");

ASSERT(ORIGIN(RAM) % 4 == 0, "
ERROR: the start of the RAM region must be 4-byte aligned");
...
ERROR: stack start address is not 8-byte aligned.
...
ERROR: end of stack is not 4-byte aligned");

ASSERT(_stack_start >= _stack_end, "
ERROR: stack end address is not below stack start.");
...
BUG: the reset vector is missing");

ASSERT(__eexceptions == ADDR(.vector_table) + 0x40, "
BUG: the exception vectors are missing");

ASSERT(SIZEOF(.vector_table) > 0x40, "
ERROR: The interrupt vectors are missing.
...
ERROR: The .text section can't be placed inside the .vector_table section
Set _stext to an address greater than the end of .vector_table (See output of `nm`)");

ASSERT(_stext > ORIGIN(FLASH_BOOT) && _stext < ORIGIN(FLASH_BOOT) + LENGTH(FLASH_BOOT), "
ERROR: The .text section must be placed inside the FLASH_BOOT memory.
Set _stext to an address within the FLASH_BOOT region.");

/* # Other checks */
ASSERT(SIZEOF(.got) == 0, "
ERROR: .got section detected in the input object files
